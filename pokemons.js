...
function RenderCard(card) {
	...
	card.forEach(elem => {
	  ...
	  card.innerHTML = `
		...
		<div class="info">
			<p><strong>Candy count:</strong> ${elem.candy_count}</p>
			<p><strong>Weight:</strong> ${elem.weight}</p>
			<p class="weakness">${elem.weaknesses}</p>
		</div>
		<div class="time"> Time: ${elem.spawn_time}</div>`
...
options.addEventListener('change', () => {
	let wrapper = document.querySelector('.cards')
	wrapper.innerHTML = ''
	let filter = pokemons.filter(elem => elem.type.includes(options.value))
	RenderCard(filter)
